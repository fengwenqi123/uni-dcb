<template>
	<div class="container" v-if="normalList">
		<scroll-view class="scroll-view_H" scroll-y>
			<div class="main">
				<div class="foodsInfo items">
					...
					<div class="title tag tag1">自定义菜品</div>
					<ul>
						<li v-for="(item, index) in normalList" :key="index">
							<div class="item" v-if="item.status === 4 && item.dishCount > 0">
								<div class="img"></div>
		...
		<div class="button"><van-button size="large" class="submit" type="info" @click="submitOrder">确认订单</van-button></div>
	...
	methods: {
		...
		_editOrderActive() {
			editOrderActive({
				...
				setTimeout(() => {
					wx.switchTab({
						url: '/pages/business/confirmed/confirmed'
					});
				}, 1500);
			});
		},
